	...
	return (
		<form className={classCSS} onSubmit={(e) => method(e)}>
			...
			<div>
				...
				<input onChange={(e) => setPseudo(e.target.value)} id={id} type={"text"} value={pseudo} required />
			</div>

			<div>
				<label htmlFor={id2}>Votre password</label>
				<input onChange={(e) => setPassword(e.target.value)} id={id2} type={"password"} value={password} required />
			</div>
